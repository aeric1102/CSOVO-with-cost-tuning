...
void train(void)
{
	...
	if((pid = fork()) < 0){
		...
		if(execl("./svm-train", "./svm-train", "-s", "5", "-l", "3.cost", "../data/new-thyroid", NULL) < 0){
...
void predict(void)
{
	...
	if((pid = fork()) < 0){
		...
		if(execl("./svm-predict", "./svm-predict", "-l", "3.cost", "../data/new-thyroid.t", "new-thyroid.model", "x", NULL) < 0){
